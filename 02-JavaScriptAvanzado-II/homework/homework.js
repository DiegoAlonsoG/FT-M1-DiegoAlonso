...
function cacheFunction(cb) {
  // Usa closures para crear un caché para la función cb. (1)
  // la función que retornas debe aceptar un solo argumento e invocar a cb con ese argumento (2)
  // cuando la función que hayas retornado es invocada de nuevo, debería guardar el argumento y el resultado de la invocacion (3)
  ...
  // debería retornar el resultado (previamente guardado) (4)
  // Ejemplo:
  ...
  // usá hasOwnProperty!
  var cache = {};  // (1)
  return function(x) { // (2a)
    if (cache.hasOwnProperty(x)) { //(4)
      return cache[x];
    }
    cache[x] = cb(x); // (3)
    return cache[x]; // (2b) chache[x] = cb(x)
  }
}
